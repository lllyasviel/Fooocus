...
shared.gradio_root = gr.Blocks(
    title=f'玩咖绘图 {fooocus_version.version} ' + ('' if args_manager.args.preset is None else args_manager.args.preset),
...
with shared.gradio_root:
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Row(elem_classes='gen_ai_row'):
                gr.Column(scale=15, min_width=0)
                gen_ai_checkbox = gr.Checkbox(label='AI Generate', value=True, container=False,
                                                   elem_classes='gen_ai_checkbox', min_width=40)
            with gr.Row(elem_classes='type_row', visible=True) as gen_ai_panel:
                with gr.Column(scale=17):
                    translate = gr.Textbox(show_label=False, placeholder="输入中文提示词.",
                                           value="",
                                           container=False, autofocus=True, elem_classes='type_row', lines=1024)
                with gr.Column(scale=3, min_width=0):
                    # 创建一个名为"Generate"的按钮控件，样式类为"type_row"，可见
                    translate_button = gr.Button(label="translate", value="AI Translate", elem_classes='type_row_half',
                                                 elem_id='translate_button', visible=True)
                    tag_translate_button = gr.Button(label="tag_translate", value="AI Tag Translate", elem_classes='type_row_half',
                                                 elem_id='tag_translate_button', visible=True)
                gen_ai_checkbox.change(lambda x: gr.update(visible=x), inputs=gen_ai_checkbox,
                                            outputs=gen_ai_panel, queue=False)
            with gr.Row(elem_classes='type_row'):
                ...
                with gr.Column(scale=3, min_width=0):
                    ...
                    def tag_translate_clicked(prompt ,translate):
                        from fcbh.gen_ai import alibaba_translate
                        translated = alibaba_translate(translate).replace('"', '').rstrip('.')
                        if prompt == '':
                            return translated
                        if translate == '':
                            return prompt
                    ...
                    def translate_clicked(translate):
                        from fcbh.gen_ai import spark_translate
                        translated_text = spark_translate(translate).replace('"', '').rstrip('.')
                        return translated_text

                    tag_translate_button.click(tag_translate_clicked, inputs=[prompt, translate], outputs=[prompt], queue=False)
                    translate_button.click(translate_clicked, inputs=[translate], outputs=[prompt], queue=False)
            ...
            with gr.Row(visible=False) as image_input_panel:
                with gr.Tabs():
                    ...
                    with gr.TabItem(label="Remove Background (beta)") as rembg_tab:
                        with gr.Row():
                            with gr.Column():
                                rembg_input_image = grh.Image(label='Drag above image to here', source='upload',
                                                              type='numpy', elem_id='remove_bg', show_label=True)
                            with gr.Column():
                                with gr.Row():
                                    # rembg_model = gr.Dropdown(label="Remove Background Models", show_label=True,
                                    #                           choices=modules.path.available_rembg_models,
                                    #                           value=modules.path.default_rembg_model_name)
                                    rembg_method = gr.Radio(label='Remove Background: ', choices=flags.rembg_list,
                                                          value=flags.disabled)
                                    with gr.Row():
                                        return_mask = gr.Checkbox(label="Return Mask", value=False, show_label=True)
            ...
            rembg_tab.select(lambda: 'rembg', outputs=current_tab, queue=False, _js=down_js, show_progress=False)
            ...
            with gr.Tab(label='Setting'):
                with gr.Row():
                    ...
                    image_number = gr.Slider(label='Image Number', minimum=1, maximum=32, step=1,
                                             value=modules.path.default_image_number)
                sharpness = gr.Slider(label='Sampling Sharpness', minimum=0.0, maximum=30.0, step=0.001,
                                      value=modules.path.default_sample_sharpness,
                                      info='Higher value means image and texture are sharper.')
                guidance_scale = gr.Slider(label='Guidance Scale', minimum=1.0, maximum=30.0, step=0.01,
                                           value=modules.path.default_cfg_scale,
                                           info='Higher value means style is cleaner, vivider, and more artistic.')
                aspect_ratios_selection = gr.Dropdown(label='Aspect Ratios', choices=modules.path.available_aspect_ratios,
                                                      value=modules.path.default_aspect_ratio, info='width × height',
                                                      show_label=True, multiselect=False, allow_custom_value=False)
            ...
            with gr.Tab(label='Develop'):
                ...
                with gr.Column(visible=False) as dev_tools:
                    with gr.Tab(label='Developer Debug Tools'):
                        ...
                        inpaint_engine = gr.Dropdown(label='Inpaint Engine', value='v2.5', choices=['v1', 'v2.5'],
        ...
        ctrls += ip_ctrls
        ctrls += [rembg_method, rembg_input_image, return_mask]

        generate_button.click(lambda: (gr.update(visible=True, interactive=True), gr.update(visible=True, interactive=True), gr.update(interactive=False), gr.update(interactive=False), gr.update(visible=False), []), outputs=[stop_button, skip_button, translate_button, tag_translate_button, generate_button, gallery]) \
            .then(fn=refresh_seed, inputs=[seed_random, image_seed], outputs=image_seed) \
            .then(advanced_parameters.set_all_advanced_parameters, inputs=adps) \
            .then(fn=generate_clicked, inputs=ctrls, outputs=[progress_html, progress_window, progress_gallery, gallery]) \
            .then(lambda: (gr.update(interactive=True), gr.update(interactive=True), gr.update(visible=True), gr.update(visible=False), gr.update(visible=False)), outputs=[translate_button, tag_translate_button, generate_button, stop_button, skip_button]) \
        ...
        for notification_file in ['notification.ogg', 'notification.mp3']:
            if os.path.exists(notification_file):
                ...
                break
    # footer = gr.HTML(value=modules.html.versions_html(), elem_id='footer', elem_classes='footer')
