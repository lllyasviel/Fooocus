...
def queue_click(*args):
    ...
    Inputs:
        - prompt, negative_prompt,
          style_selections, performance_selection, aspect_ratios_selection,
          image_number, image_seed,
...
def processing_state():
    ...
    while worker.running_tasks or len(worker.async_tasks) or task:
        ...
        try:
            new_task = worker.running_tasks[0]
        except IndexError:
            new_task = None

        if task and task != new_task:
            task = new_task
            yield (
                *update_state(),
                gr.update(),
        ...
        with gr.Column(scale=2):
            ...
            with gr.Row():
                prompt = gr.Textbox(
                    ...
                    scale=3,
                    lines=3,
                    value=modules.config.default_prompt,
                )
        ...
        with gr.Column(scale=1) as advanced_column:
            with gr.Tab(label='Setting'):
                ...
                performance_selection.change(lambda r: gr.update(interactive=r == 'Custom'), inputs=[performance_selection],
                                             outputs=[custom_steps], queue=False, show_progress=False)
