...
class SamplerDPMPP_3M_SDE:
    ...
    def get_sampler(self, eta, s_noise, noise_device):
        ...
        sampler = ldm_patched.modules.samplers.ksampler(sampler_name, {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerEulerAncestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = ldm_patched.modules.samplers.ksampler("euler_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerLMS:
    ...
    def get_sampler(self, order):
        sampler = ldm_patched.modules.samplers.ksampler("lms", {"order": order})
        return (sampler, )
...
class SamplerDPMAdaptative:
    ...
    def get_sampler(self, order, rtol, atol, h_init, pcoeff, icoeff, dcoeff, accept_safety, eta, s_noise):
        sampler = ldm_patched.modules.samplers.ksampler("dpm_adaptive", {"order": order, "rtol": rtol, "atol": atol, "h_init": h_init, "pcoeff": pcoeff,
...
class Noise_RandomNoise:
    ...
    def generate_noise(self, input_latent):
        ...
        return ldm_patched.modules.sample.prepare_noise(latent_image, self.seed, batch_inds)
...
class Guider_Basic(ldm_patched.modules.samplers.CFGGuider):
...
class CFGGuider:
    ...
    def get_guider(self, model, positive, negative, cfg):
        guider = ldm_patched.modules.samplers.CFGGuider(model)
        guider.set_conds(positive, negative)
        guider.set_cfg(cfg)
        return (guider,)

class Guider_DualCFG(ldm_patched.modules.samplers.CFGGuider):
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None):
        ...
        out = ldm_patched.modules.samplers.calc_cond_batch(self.inner_model, [negative_cond, middle_cond, self.conds.get("positive", None)], x, timestep, model_options)
        return ldm_patched.modules.samplers.cfg_function(self.inner_model, out[1], out[0], self.cfg2, x, timestep, model_options=model_options, cond=middle_cond, uncond=negative_cond) + (out[2] - out[1]) * self.cfg1
