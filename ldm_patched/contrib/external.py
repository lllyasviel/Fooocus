...
import ldm_patched.utils.latent_visualization as latent_preview
import ldm_patched.utils.node_helpers as node_helpers
...
class CheckpointLoader:
    ...
    def load_checkpoint(self, config_name, ckpt_name, output_vae=True, output_clip=True):
        ...
        return ldm_patched.modules.sd.load_checkpoint(config_path, ckpt_path, output_vae=True, output_clip=True,
...
class CheckpointLoaderSimple:
    ...
    def load_checkpoint(self, ckpt_name, output_vae=True, output_clip=True):
        ckpt_path = folder_paths.get_full_path("checkpoints", ckpt_name)
        out = ldm_patched.modules.sd.load_checkpoint_guess_config(ckpt_path, output_vae=True, output_clip=True,
                                                    embedding_directory=folder_paths.get_folder_paths("embeddings"))
        return out[:3]
...
class DiffusersLoader:
    ...
    def load_checkpoint(self, model_path, output_vae=True, output_clip=True):
        ...
        return ldm_patched.modules.diffusers_load.load_diffusers(model_path, output_vae=output_vae, output_clip=output_clip,
                                                   embedding_directory=folder_paths.get_folder_paths("embeddings"))
...
class VAELoader:
    ...
    def load_taesd(name):
        ...
        enc = ldm_patched.modules.utils.load_torch_file(folder_paths.get_full_path("vae_approx", encoder))
        for k in enc:
            sd["taesd_encoder.{}".format(k)] = enc[k]

        dec = ldm_patched.modules.utils.load_torch_file(folder_paths.get_full_path("vae_approx", decoder))
        for k in dec:
...
class CLIPLoader:
    ...
    def load_clip(self, clip_name, type="stable_diffusion"):
        clip_type = ldm_patched.modules.sd.CLIPType.STABLE_DIFFUSION
        if type == "stable_cascade":
            clip_type = ldm_patched.modules.sd.CLIPType.STABLE_CASCADE

        clip_path = folder_paths.get_full_path("clip", clip_name)
        clip = ldm_patched.modules.sd.load_clip(ckpt_paths=[clip_path],
...
class DualCLIPLoader:
    ...
    def load_clip(self, clip_name1, clip_name2):
        ...
        clip = ldm_patched.modules.sd.load_clip(ckpt_paths=[clip_path1, clip_path2],
                                  embedding_directory=folder_paths.get_folder_paths("embeddings"))
        return (clip,)
...
class LatentBlend:
    ...
    def blend(self, samples1, samples2, blend_factor: float, blend_mode: str = "normal"):
        ...
        if samples1.shape != samples2.shape:
            ...
            samples2 = ldm_patched.modules.utils.common_upscale(samples2, samples1.shape[3], samples1.shape[2], 'bicubic',
                                                  crop='center')
...
class ImageBatch:
    ...
    def batch(self, image1, image2):
        if image1.shape[1:] != image2.shape[1:]:
            image2 = ldm_patched.modules.utils.common_upscale(image2.movedim(-1, 1), image1.shape[2], image1.shape[1], "bilinear",
                                                "center").movedim(1, -1)
